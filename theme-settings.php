 ...
 */
function hippelicious_settings($settings) {
  // Add js & css
  drupal_add_css('misc/farbtastic/farbtastic.css', 'module', 'all', FALSE);
  drupal_add_js('misc/farbtastic/farbtastic.js');
  drupal_add_js(drupal_get_path('theme', 'hippelicious') .'/js/settings.js');
  drupal_add_css(drupal_get_path('theme', 'hippelicious') .'/css/settings.css');
  ...
  if ($file = file_save_upload('background_file', array('file_validate_is_image' => array()))) {
    ...
    $filename = 'hippelicious_background.'. $parts['extension'];
  ...
  $form['style'] = array(
    '#title' => t('Styles'),
    '#type' => 'select',
    '#options' => hippelicious_get_styles(),
    '#default_value' => !empty($settings['style']) ? $settings['style'] : 'sea',
  );
  $form['custom'] = array(
    '#tree' => FALSE,
    '#type' => 'fieldset',
    '#attributes' => array('class' => ($form['style']['#default_value'] == 'custom') ? 'hippel-custom-settings' : 'hippel-custom-settings hidden'),
  );
  ...
  $form['custom']['background_color'] = array(
    ...
    '#suffix' => '<div id="hippel-colorpicker"></div>',
  );
